package com.damon.test.thread.cyclicBarrier;
 ...
 *
 * Created by ZhouJW on 2015/5/8 17:37.
 */
...
public class MatrixMock {
    ...
    //3.   实现类的构造函数。此构造函数将接收矩阵的行数，行的长度，
    // 和我们将要查找的数字作为参数。3个参数全部int 类型。
...
//6.   最后，在操控台打印一条信息，表示查找的数字在生成的矩阵里的出现次数。此信息是用来检查线程们获得的正确结果的。
        System.out.printf("Mock: There are %d ocurrences of number in generated data.%d\n", counter, number); //译者注：把字符串里的number改为%d.
    }
...
//24. 调用 CyclicBarrier 对象的 await() 方法 ，由于可能抛出的异常，
