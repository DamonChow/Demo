 ...
 * @since 2017/1/3 9:30
 */
public class PhilosopherForkTest {
...
class Philosopher implements Runnable {
    ...
    public void run() {
        while (true) {
            try {
                PhilosopherForkTest.counter.acquire();
                boolean leftFirst = index % 2 == 0;
                PhilosopherForkTest.putOnFork(index, leftFirst);
                System.out.println(name + "正在吃意大利面（通心粉）...");   // 取到两个叉子就可以进食
                PhilosopherForkTest.putDownFork(index, leftFirst);
                PhilosopherForkTest.counter.release();
